...
function winningSequence(r0, r1, ri, c0, c1, ci) {
...
function winningSituation() {
  ...
  for (let i = 0; i < N; i++) {
    if (winningSequence(i, i+1, 0, 0, N, 1)) {
      return true
    }
  }

  for (let i = 0; i < N; i++) {
    if (winningSequence(i, i+1, 0, 0, N, 1) || winningSequence(0, N, 1, i, i+1, 0)) {
      return true
    }
  }

  if (winningSequence(0, N, 1, 0, N, 1) || winningSequence (N-1, -1, -1, 0, N, 1)) {
    return true
  }
  return false
}